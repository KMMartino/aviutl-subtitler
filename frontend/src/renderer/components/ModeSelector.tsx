...
export default function ModeSelector({ workflow, onChange, disabled = false }: { workflow: WorkflowName; onChange(value: WorkflowName): void; disabled?: boolean }) {
  ...
  return (
    <div className="mode-selector">
      ...
      {mode.longStream ? <span className="segmented"><button disabled className="active"><HardDrive size={15} /> {t("mode.local")}</button></span> : <div className="segmented" aria-label={t("mode.processingLocation")}>
        <button disabled={disabled || mode.longStream} className={!mode.hosted ? "active" : ""} onClick={() => onChange(modeToWorkflow(false, mode.longStream))}><HardDrive size={15} /> {t("mode.local")}</button>
