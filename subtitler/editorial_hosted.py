...
def _analyze_editorial_visual_windows(
    *,
    ...
    """Run the editorial profile through the shared resumable visual service."""
    return analyze_visual_windows(
        media_path=media_path, duration_sec=duration_sec, detail=detail, ffmpeg=ffmpeg,
        sampling_scale=sampling_scale, model=model, reasoning_effort=reasoning_effort,
        output_locale=output_locale, editorial_context=editorial_context,
        progress_path=progress_path, diagnostics_path=diagnostics_path, progress=progress,
        max_workers=max_workers, window_interval_sec=window_interval_sec,
        profile="editorial", stage_version=EDITORIAL_STAGE_VERSIONS["visual_learning"],
        provider_factory=OpenAIEditorialVisualProvider, analyze=analyze_media,
    )
